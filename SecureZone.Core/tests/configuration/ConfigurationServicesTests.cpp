...
void createsAndUpdatesValidZone() {
    ...
    assert(service.assignXProtectEvent(zone.zoneId, "SecureZone.LineCrossing").succeeded());
    assert(zones.zones.at(zone.zoneId).xprotectEventName == "SecureZone.LineCrossing");
...
void rejectsInvalidZoneConfiguration() {
    ...
    auto missingXProtectEvent = validZone();
    missingXProtectEvent.xprotectEventName.clear();
    assert(
        service.create(missingXProtectEvent).status
        == configuration::ZoneConfigurationStatus::InvalidZone
    );
    ...
    assert(
        service.assignXProtectEvent("ZONE-001", "").status
        == configuration::ZoneConfigurationStatus::InvalidZone
    );
